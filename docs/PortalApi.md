...
[**portal_finalize_synchronous**](PortalApi.md#portal_finalize_synchronous) | **POST** /{tenantUrlCode}/api/v2/Portal/upload/{uploadId}/finalizeSynchronous | Finalizes a data import with a file uploaded using &#x60;upload&#x60;.
...
# **portal_finalize_synchronous**
> portal_finalize_synchronous(body, upload_id, tenant_url_code)

Finalizes a data import with a file uploaded using `upload`.

Unlike `upload/{uploadId}/finalize`, this call will return errors if there is a failure at any step of the upload and will wait until the end of the process to return 200.
...
try:
    # Finalizes a data import with a file uploaded using `upload`.
    api_instance.portal_finalize_synchronous(body, upload_id, tenant_url_code)
except ApiException as e:
    print("Exception when calling PortalApi->portal_finalize_synchronous: %s\n" % e)
```
...
Will return 200 if the upload is started succesfully, but there won't be any indication if there is a failure later on. To see errors that occur during the upload, use `upload/{uploadId}/finalizeSynchronous`.
...
> FileDescription portal_get_definition(upload_id, restrict_for, tenant_url_code)
...
api_instance = cartovista_cloud_clients.PortalApi(cartovista_cloud_clients.ApiClient(configuration))
upload_id = 'upload_id_example' # str | 
restrict_for = cartovista_cloud_clients.RestrictFor() # RestrictFor | 
tenant_url_code = 'tenant_url_code_example' # str | 

try:
    # Gets a description of the file's content.
    api_response = api_instance.portal_get_definition(upload_id, restrict_for, tenant_url_code)
    pprint(api_response)
except ApiException as e:
